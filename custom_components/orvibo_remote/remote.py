...
from .const import CONF_ENABLE_RF, CONF_MODEL_HINT, DATA_CLIENTS, DEFAULT_NAME
...
async def async_setup_entry(
    ...
    async_add_entities([OrviboRemote(name=name, client=client, timeout=15)])
...
class OrviboRemote(RemoteEntity):
    ...
    def __init__(self, name: str, client: OrviboClient, timeout: int = 15) -> None:
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        capabilities = self._client.detect_capabilities(
            hardware_type="irda",
            model_hint=self._client._model_hint,
            enable_rf=self._client._enable_rf,
        )
        return {
